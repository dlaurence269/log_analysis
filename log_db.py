...
def get_high_errors():
    ...
    c.execute("""
      select
        error_statuses.date,
        cast(error_statuses.count as decimal(18,1)) /
        cast(total_statuses.count as decimal(18,1)) as final_count
      ...
      join(select time::date as date, count(status)
        from log
        group by date) as total_statuses
        on error_statuses.date = total_statuses.date
      where cast(error_statuses.count as decimal(18,1)) /
      cast(total_statuses.count as decimal(18,1)) > .01
